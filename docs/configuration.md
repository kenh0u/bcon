...
3. `~/.config/bcon/config.toml` — per-user override. Overlays the site
...
| `bcon --init-config=/tmp/x.toml` | `/tmp/x.toml` (load via `BCON_CONFIG`, see below) |
| `bcon --init-config=~/foo.toml` | `$HOME/foo.toml` (tilde-expanded; load via `BCON_CONFIG`, see below) |

The `system` token requires write access to `/etc/bcon/`; run with `sudo`.
...
## Single-file bypass: `BCON_CONFIG`

The environment variable `BCON_CONFIG`, when set to a path that exists,
...
1. **Loading an ad-hoc config generated by a path target.** Files
   ...
   automatically. `BCON_CONFIG` is the link between such ad-hoc
   destinations and runtime use.
2. **Debugging.** `BCON_CONFIG=/dev/null bcon` loads no file but parses
   defaults, so you can A/B-compare against the built-in baseline
   without touching `/etc/bcon/` or `~/.config/bcon/`.

Example:

```bash
bcon --init-config=~/foo.toml,vim       # generate the file
